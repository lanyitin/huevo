    package tw.lanyitin.huevo

import java.util.regex.Pattern
import scala.annotation.tailrec

object typeDef {
  type Tokenizer = Tuple2[Pattern, TokenFn]
  type TokenFn = (String, Integer, Integer) => Token
}

sealed trait ScannerMode {
  import typeDef._
  def tokenizer: List[Tuple2[Pattern, TokenFn]]
}

case object NormalMode extends ScannerMode {
  import TokenType._
  import typeDef._
  def tokenizer: List[Tuple2[Pattern, TokenFn]] =
    (Pattern.compile("^(def)"), (txt: String, line: Integer, col: Integer) => new Token(DefToken, txt, line, col)) ::
    (Pattern.compile("^(if)"), (txt: String, line: Integer, col: Integer) => new Token(IfToken, txt, line, col)) ::
    (Pattern.compile("^(else)"), (txt: String, line: Integer, col: Integer) => new Token(ElseToken, txt, line, col)) ::
    (Pattern.compile("^([a-zA-Z][a-zA-Z0-9]*)"), (txt: String, line: Integer, col: Integer) => new Token(IdentifierToken, txt, line, col)) ::
    (Pattern.compile("^(\\r\\n|\\n|\\r)"), (txt: String, line: Integer, col: Integer) => new Token(NewLineToken, txt, line, col)) ::
    (Pattern.compile("^([\\t ]+)"), (txt: String, line: Integer, col: Integer) => new Token(SpaceToken, txt, line, col)) ::
    (Pattern.compile("^(\\d+)"), (txt: String, line: Integer, col: Integer) => new Token(NumberToken, txt, line, col)) ::
    (Pattern.compile("^(,)"), (txt: String, line: Integer, col: Integer) => new Token(CommaToken, txt, line, col)) ::
    (Pattern.compile("^(:)"), (txt: String, line: Integer, col: Integer) => new Token(ColumnToken, txt, line, col)) ::
    (Pattern.compile("^(\\+|-|\\*|\\/|>|<|=)|and|or|>=|<="), (txt: String, line: Integer, col: Integer) => new Token(OperatorToken, txt, line, col)) ::
    (Pattern.compile("^(\\{)"), (txt: String, line: Integer, col: Integer) => new Token(LCurlyBracket, txt, line, col)) ::
    (Pattern.compile("^(\\})"), (txt: String, line: Integer, col: Integer) => new Token(RCurlyBracket, txt, line, col)) ::
    (Pattern.compile("^(\\()"), (txt: String, line: Integer, col: Integer) => new Token(LParanToken, txt, line, col)) ::
    (Pattern.compile("^(\\))"), (txt: String, line: Integer, col: Integer) => new Token(RParanToken, txt, line, col)) ::
    Nil
}

case object StringMode extends ScannerMode {
  // TODO: implement string mode
  def tokenizer = ???
}

case object CommentMode extends ScannerMode {
  // TODO: implement comment mode
  def tokenizer = ???
}


case class ScannerState(position: Integer, col: Integer, line: Integer, mode: ScannerMode)

case class Scanner(val content: String, state: ScannerState, print: (String) => Unit) {
  import TokenType._
  import typeDef._
  def nextToken(): (Token, Scanner) = {
    // TODO: rewrite loop with built-in operation like collectFirst
    @tailrec
    def loop(tokenizer: List[Tokenizer]): Token = {
      if (tokenizer.isEmpty) {
        new Token(UnexpectedToken, "<UnexpectedToken>")
      } else {
        val matcher = tokenizer.head._1.matcher(content.substring(state.position))
        if (matcher.find()) {
          tokenizer.head._2(matcher.group(0), state.line, state.col)
        } else {
          loop(tokenizer.tail)
        }
      }
    }
    
    if (state.position >= content.size) {
      (new Token(EOFToken, "<EOF>", state.line, state.col), this)
    } else {
      val result = loop(state.mode.tokenizer)
      // TODO: refactoring follow code segment to ScannerMode or ScannerState
      result.tokenType match {
        case OperatorToken => (result, Scanner(content, state.copy(position=state.position + result.txt.size,col=state.col+result.txt.size), print))
        case IdentifierToken => (result, Scanner(content, state.copy(position=state.position + result.txt.size,col=state.col+result.txt.size), print))
        case SpaceToken => Scanner(content, state.copy(position=state.position + result.txt.size,col=state.col+result.txt.size), print).nextToken()
        case NumberToken => (result, Scanner(content, state.copy(position=state.position + result.txt.size,col=state.col+result.txt.size), print))
        case StringToken => (result, Scanner(content, state.copy(position=state.position + result.txt.size,col=state.col+result.txt.size), print))
        case NewLineToken => (result, Scanner(content, state.copy(position=state.position + result.txt.size, col=0, line=state.line+1), print))
        case LParanToken => (result, Scanner(content, state.copy(position=state.position + 1,col=state.col+result.txt.size), print))
        case RParanToken => (result, Scanner(content, state.copy(position=state.position + 1,col=state.col+result.txt.size), print))
        case LCurlyBracket => (result, Scanner(content, state.copy(position=state.position + 1,col=state.col+result.txt.size), print))
        case RCurlyBracket => (result, Scanner(content, state.copy(position=state.position + 1,col=state.col+result.txt.size), print))
        case ColumnToken => (result, Scanner(content, state.copy(position=state.position + 1,col=state.col+result.txt.size), print))
        case CommaToken => (result, Scanner(content, state.copy(position=state.position + 1,col=state.col+result.txt.size), print))
        case DefToken => (result, Scanner(content, state.copy(position=state.position + 3,col=state.col+result.txt.size), print))
        case IfToken => (result, Scanner(content, state.copy(position=state.position + 2,col=state.col+result.txt.size), print))
        case ElseToken => (result, Scanner(content, state.copy(position=state.position + 4,col=state.col+result.txt.size), print))
        case EOFToken => (result, this)
        case UnexpectedToken => (result, this)
      }
    }
  }

  private[this] def _loop(scanner: Scanner, acc: List[Token], p: (Token) => Boolean): (Scanner, List[Token]) = {
    val (token, next_state) = scanner.nextToken
    if (!p(token) || token.tokenType == EOFToken) {
      (scanner, (token :: acc).reverse)
    } else {
      _loop(next_state, token :: acc, p)
    }
  }

  def takeWhile(p: (Token) => Boolean): List[Token] = {
    _loop(this, Nil, p)._2
  }


  def skip(p: (Token) => Boolean): Scanner = {
    _loop(this, Nil, p)._1
  }

  def take(num: Int): List[Token] = {
    var n = num
    val fn = (_: Token) => {n-=1;n > 0}
    takeWhile(fn)
  }
}


object Scanner {
  // keep tracking of how many token be scanned
  var tokenId = 0

  def apply(txt: String, print: (String) => Unit=(_) => Unit): Scanner = {
    Scanner(txt, ScannerState(0, 0, 0, NormalMode), print)
  }
}
